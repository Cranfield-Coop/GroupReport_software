...
def app():
    ...
    if uploaded_file_pysindy is not None:
        st.session_state.csv_file = uploaded_file_pysindy
    ...
    if st.session_state.csv_file is not None:
        ...
        if mode_choice == 'Data with interpolation':
            nb_interpolation = st.number_input(
                "Please enter the number of data points to be generated with interpolation:", min_value=5000, max_value=10000, step=1000, key='nb_interp')
            if nb_interpolation < len(df):
                st.error(
                    f"The number of data points to be generated with interpolation must be greater than the number of data points in the csv file ({len(df)})")
                return None
        ...
        if data is not None:
            ...
            if len(Reynolds_Numbers) > 1:
                ...
                ratio = st.number_input("Please enter the ratio to split the data into training and testing datasets:",
                                        value=0.8, min_value=0.0, max_value=1.0, format="%.2f")
                X_1 = X[0]
                y_1 = y[0]
            ...
            if st.session_state.run_model:
                ...
                if sparsity == -1:
                    ...
                    if len(Reynolds_Numbers) > 1:
                        ...
                        df_sim = pd.DataFrame(
                            {"y^+": solution.t, 'U': solution.y[0], "u'u'": solution.y[1], "v'v'": solution.y[2], "w'w'": solution.y[3], "u'v'": solution.y[4]})
                        df_pred = data[f"df_{Reynolds_Numbers[0]}"][data[f"df_{Reynolds_Numbers[0]}"]['y^+'].isin(
                            solution.t)]
                        mae_sim = mean_absolute_error(df_pred[["U", "u'u'", "v'v'", "w'w'", "u'v'"]].values, df_sim[[
                                                      "U", "u'u'", "v'v'", "w'w'", "u'v'"]].values)
                    if mae_sim < 1:
